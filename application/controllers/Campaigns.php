...
class Campaigns extends CI_Controller {
    ...
    public function list_campaign_info() {
        ...
        foreach ($data as $k => $d) {
            ...
            if ($d['message_status'] == 'failed') {
                if (!empty($d['response'])) {
                    $response = json_decode($d['response'], 1);
                    if (isset($response['error']) && !empty($response['error'])) {
                        if (isset($response['error']['message']) && !empty($response['error']['message'])) {
                            $final['data'][$k]['error_message'] = $response['error']['message'];
                        }
                    } else if (!empty($d['error_message'])) {
                        $final['data'][$k]['error_message'] = $d['error_message'];
                    }
                }
            }
        }
    ...
    public function create_recampaign($campaign_id) {
        $this->data['campaign_id'] = $campaign_id;
        $user_id = $this->session->userdata('id');
        $where = 'user_id = ' . $this->db->escape($user_id) . ' AND temp_status = "APPROVED" AND is_deleted = 0';
        $this->data['templates'] = $templates = $this->CMS_model->get_result(tbl_templates, $where);

        $this->template->load('default_home', 'Campaigns/create_recampaign', $this->data);
    }

    public function get_contact_for_recampaign(){
        $campaign_id = base64_decode($this->input->post('campaign_id'));
        $campaign_for = $this->input->post('campaign_for');
        $contact_arr = $this->Campaigns_model->get_recampaign_contact($campaign_id, $campaign_for);
        
        echo json_encode(array('contacts' => !empty($contact_arr) ? count($contact_arr) : 0));
    }
    
    public function save_recampaign() {
        $this->form_validation->set_rules('recampaign_for', 'Recampaign For', 'trim|required');
        $this->form_validation->set_rules('campaign_name', 'Name', 'trim|required');
        $this->form_validation->set_rules('campaign_template', 'Template', 'trim|required');
        if ($this->input->post('notification_campaign') == 'schedule_campaign') {
            $notification = $this->input->post('notification_date');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $campaign_id = base64_decode($this->input->post('campaign_id'));
            $campaign_data['user_id'] = $user_id = $this->session->userdata('id');
            $recampaign_for = $this->input->post('recampaign_for');
            if (!empty($user_id)) {
                $campaign_data['campaign_name'] = $campaign_name = $post['campaign_name'];
                $campaign_data['template_id'] = $input_arr['template_id'] = $post['campaign_template'];
                ...
                $conatcts_arr = $this->Campaigns_model->get_recampaign_contact($campaign_id, $recampaign_for);
