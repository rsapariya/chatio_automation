...
class ReplyResponse_model extends CI_Model {
    ...
    public function get_all_reply_responses($count = null, $user_id = 0) {
        ...
        $select = 'rr.id,@a:=@a+1 AS test_id,rr.name,rr.mobile_number,rr.response,DATE_FORMAT(rr.created,"%d %b %Y <br> %l:%i %p") AS created_at,rr.user_id';
        $this->db->select($select, false);
        $keyword = $this->input->post('search');
        $trigger_text = $this->input->post('trigger_text');
        $query_time = $this->input->post('query_time');
        if (!empty($keyword['value'])) {
            $this->db->having('rr.response LIKE "%' . $keyword['value'] . '%" OR rr.name LIKE "%' . $keyword['value'] . '%" OR rr.mobile_number LIKE "%' . $keyword['value'] . '%" OR created_at LIKE "%' . $keyword['value'] . '%"');
        }
        ...
        if (is_null($count)):
            ...
            $res_data = $this->db->get(tbl_button_reply_logs . ' rr')->num_rows();
        endif;
        return $res_data;
    }
